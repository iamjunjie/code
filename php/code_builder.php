...
$tmpstr .= "public function getList($"."data) 
    ";
$tmpstr .= "{ 
        ";
$tmpstr .= "//参数 
...
$tmpstr .= "
        \$param['creator_id'] = \$param['modifier_id'] = getArrVal(\$data,\"member_id\");
...
$tmpstr .=" 
        \$param['modifier_id'] = getArrVal(\$data,\"member_id\");
...
#############################构建model结束构建控制器开始######################################################
    ...
    $controllerFile = fopen($controller_name.".php","w");
    $controllerStr = "<?php
...
##################创建Controller结束
    $logicFile = fopen($logic_name.".php","w");
    $logicStr = "<?php
